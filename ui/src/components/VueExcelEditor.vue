...
export default defineComponent({
    ...
    methods: {
        ...
        componentTabInto(e) {
            if (e.keyCode === 9) {
                if (!this.moveInputSquare(this.currentRowPos, this.currentColPos)) this.moveInputSquare(0, 0);
            }
        },
        reset() {
            this.errmsg = {};
        ...
        calTable() {
            ...
            */
                        ...
                        filterColumnList.forEach((k) => {
                            ...
                            if (this.fields[k].type === 'number' && filter[k].type <= 4) content[k] = val;
                            else content[k] = typeof val === 'undefined' || val === null ? '' : String(val).toUpperCase();
                        });
        ...
        filterGrouping(rec, i, table) {
            ...
            this.fields.forEach((field) => {
                ...
                if (field.grouping && rec[name] === prec[name]) {
                    if (field.grouping === 'collapse' && this.ungroup[field.name + rec[name]] !== true) result = false;
                    else if (field.grouping === 'expand' && this.ungroup[field.name + rec[name]]) result = false;
                }
            });
            return result;
        },
        ...
        renderColumnCellStyle(field, record) {
            ...
            if (record && field.color) result.color = typeof field.color === 'function' ? field.color(record) : field.color;
            return result;
        },
        localeDate(d) {
        ...
        columnAutoWidth(name) {
            ...
            this.table.forEach((row) => {
                doFields.forEach((field) => {
                    if (row[field.name] && (!cols[field.name] || cols[field.name] < row[field.name].length)) cols[field.name] = row[field.name].length;
                });
            });
            doFields.forEach((field) => {
        ...
        showDatePickerDiv() {
            ...
            this.lazy(() => {
                ...
                if (r.right > window.innerWidth) this.$refs.dpContainer.style.left = window.innerWidth - r.width + 'px';
            });
        },
        datepickerClick() {
        ...
        vsMouseDown(e) {
            ...
            if (this.noPaging) this.tableContent.scrollTo(this.tableContent.scrollLeft, this.table.length * 24 * ratio);
            else {
                ...
                switch (e.keyCode) {
                    ...
                    case 38: // Up Arrow
                        ...
                        if (this.autocompleteInputs.length === 0) this.moveNorth();
                            ...
                            if (showTop < this.$refs.autocomplete.scrollTop) this.$refs.autocomplete.scrollTop = showTop;
                    ...
                    case 40: // Down Arrow
                        ...
                        if (this.autocompleteInputs.length === 0) this.moveSouth(e);
                            ...
                            if (this.autocompleteSelect >= 10) {
                                ...
                                if (scrollTop < showTop) this.$refs.autocomplete.scrollTop = showTop;
                            }
                        }
                        break;
                    case 13: // Enter
                    ...
                    default:
                        ...
                        if (this.currentField.allowKeys) {
                            ...
                            } else if (this.currentField.allowKeys.indexOf(e.key.toUpperCase()) === -1) return e.preventDefault();
                        }
            ...
            if (!this.noPaging) {
                ...
                if (this.height) {
                    if (this.height === 'auto') {
                        ...
                        if (p && p.scrollHeight > p.clientHeight) controlHeight += p.clientHeight - p.scrollHeight;
                    } else {
            ...
            setTimeout(() => {
                ...
                fileReader.onload = async (e) => {
                    try {
                        ...
                        if (this.fields.filter((f) => f.keyField).filter((f) => typeof importData[0][f.name] === 'undefined' && typeof importData[0][f.label] === 'undefined').length > 0) {
                        ...
                        while (pass < 2) {
                            ...
                            await Promise.all(
                                importData.map(async (line, i) => {
                                    ...
                                    if (keys.length) {
                                        // locate match record
                                        rowPos = this.table.findIndex((v) => keys.filter((f) => typeof v[f.name] !== 'undefined' && (v[f.name] === line[f.name] || v[f.name] === line[f.label])).length === keys.length);
                                        if (rowPos === -1) {
                                    ...
                                    if (rowPos === -1) rowPos = this.table.findIndex((v) => Object.keys(v).filter((f) => !f.startsWith('$')).length === 0);
    ...
    */
        rowLabelClick(e) {
            ...
            if (e.shiftKey) {
                ...
                if (this.prevSelect !== -1 && this.prevSelect !== rowPos) {
                    e.preventDefault();
                    if (rowPos > this.prevSelect) for (let i = this.prevSelect; i <= rowPos; i++) this.selectRecord(i);
                    else for (let i = rowPos; i <= this.prevSelect; i++) this.selectRecord(i);
                }
            } else {
        ...
        selectRecordByKeys(keys) {
            const rowPos = this.table.findIndex((v) => this.fields.filter((f) => f.keyField).filter((f) => v[f.name] === keys[f.name]).length === keys.length);
        ...
        mouseDown(e) {
            if (e.target.parentNode.parentNode.tagName === 'TBODY' && !e.target.classList.contains('first-col')) {
                ...
                if (colPos !== this.currentColPos || rowPos !== this.currentRowPos) this.inputBoxBlur();
                ...
                if (typeof this.currentField.cellClick === 'function') this.currentField.cellClick(this.currentCell.textContent, this.currentRecord, rowPos, colPos, this.currentField, this);
                ...
                if (e.target.offsetWidth < e.target.scrollWidth) {
                    ...
                    setTimeout(() => {
                        ...
                        if (rect.left + r.width > window.innerWidth) this.$refs.texttip.style.left = rect.right - r.width + 'px';
        ...
        cellMouseMove(e) {
            ...
            if (e.target.offsetWidth - e.offsetX < 25) {
                if (!e.target.classList.contains('readonly') && (e.target.classList.contains('select') || e.target.classList.contains('datepick'))) cursor = 'pointer';
                if (e.target.offsetWidth < e.target.scrollWidth) cursor = 'pointer';
            }
            const row = e.target.parentNode;
            const colPos = Array.from(row.children).indexOf(e.target) - 1;
            const currentField = this.fields[colPos];
            if (currentField?.type === 'action' || currentField?.grouping) cursor = 'pointer';
        ...
        moveInputSquare(rowPos, colPos) {
            ...
            if (this.currentRowPos >= 0 && this.currentRowPos < this.pagingTable.length) this.recordBody.children[this.currentRowPos].children[0].classList.remove('focus');
        ...
        inputBoxMouseMove(e) {
            let cursor = 'text';
            if (!this.currentField.readonly && (this.currentField.options || this.currentField.type === 'date') && e.target.offsetWidth - e.offsetX < 15) cursor = 'pointer';
            e.target.style.cursor = cursor;
        },
        inputBoxMouseDown(e) {
        ...
        inputCellWrite(setText, colPos, recPos) {
            ...
            if (!this.noMassUpdate && typeof this.selected[recPos] !== 'undefined') this.updateSelectedRows(field, setText);
            else this.updateCell(recPos, field, field.toValue(setText, this.table[recPos], field));
        },
        inputBoxBlur() {
            if (!this.$refs.dpContainer) return;
        ...
        async updateCell(row, field, newVal, isUndo) {
            ...
            setTimeout(() => {
                ...
                if (field.mandatory && newVal === '') transaction.err += (transaction.err ? '\n' : '') + field.mandatory;
        ...
        validateAll() {
            this.errmsg = {};
            this.rowerr = {};
            return Promise.all(this.table.map((row) => Promise.all(Object.keys(row).map((name) => this.updateCell(row, name, row[name], false)))));
        },
                ...
                const doList = async () => {
                    ...
                    if (field.options) {
                        if (field.options.constructor.name.endsWith('Function')) {
                            ...
                            if (this.inputBoxShow) list = list.filter((element) => element.toUpperCase().includes(value));
                        ...
                        for (let i = 0; i < this.modelValue.length; i++) {
                            ...
                            if (typeof rec[name] !== 'undefined' && rec[name].toString().toUpperCase().startsWith(value) && list.indexOf(rec[name]) === -1) list.push(rec[name]);
                    ...
                    this.lazy(() => {
                        ...
                        if (rect.left + r.width > window.innerWidth) this.$refs.autocomplete.style.left = rect.right - r.width + 'px';
...
<template>
    <div ref="editor" class="vue-excel-editor" :style="{ display: 'inline-block', width }">
        <div class="component-content">
            ...
            <div ref="tableContent" class="table-content" :class="{ 'no-footer': noFooter }" @scroll="tableScroll" @mouseover="mouseOver" @mouseout="mouseOut">
                <!-- Main Table -->
                <table
                    ref="systable"
                    id="systable"
                ...
                >
                    ...
                    <thead class="center-text">
                        <tr>
                            <th class="center-text first-col tl-setting" :class="{ hide: noNumCol }" style="top: 0" @contextmenu.prevent="settingClick"></th>
                            <th
                                v-for="(item, p) in fields"
                                v-show="!item.invisible"
                                :key="`th-${p}`"
                                :colspan="p === fields.length - 1 && vScroller.buttonHeight < vScroller.height ? 2 : 1"
                                :class="{ 'sort-asc-sign': sortPos == p && sortDir == 1, 'sort-des-sign': sortPos == p && sortDir == -1, 'sticky-column': item.sticky, 'no-sorting': item.noSorting }"
                            ...
                            >
                                ...
                                <div class="col-sep" @mousedown="colSepMouseDown" @mouseover="colSepMouseOver" @mouseout="colSepMouseOut">
                                    <div class="add-col-btn">+</div>
                                </div>
                            </th>
                        </tr>
                        <tr :class="{ hide: !filterRow }">
                            <td class="center-text first-col tl-filter" :class="{ hide: noNumCol }" style="vertical-align: middle; padding: 0" :style="{ top: calCellTop2 + 'px' }" @click="columnFilter = {}">
                                <span v-if="Object.keys(columnFilter).length > 0">
                                    <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="eraser" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-eraser fa-w-16 fa-sm">
                                        <path
                                            fill="currentColor"
                    ...
                    <tbody @mousedown="mouseDown">
                        ...
                        <tr v-else v-for="(record, rowPos) in pagingTable" :key="rowPos" :class="{ select: typeof selected[pageTop + rowPos] !== 'undefined' }" :style="rowStyle(record)">
                            <td
                ...
                <div v-show="processing" ref="frontdrop" class="front-drop">
                    <svg aria-hidden="true" focusable="false" data-prefix="fas" data-icon="spinner" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="svg-inline--fa fa-spinner fa-w-16 fa-spin fa-3x">
                        <path
                            fill="currentColor"
            ...
            <!-- Vertical Scroll Bar -->
            <div v-show="vScroller.buttonHeight < vScroller.height" ref="vScroll" class="v-scroll" :style="{ top: `${vScroller.top}px`, height: `${vScroller.height}px` }" @mousedown="vsMouseDown">
                <div ref="vScrollButton" class="v-scroll-button" :style="{ marginTop: `${vScroller.buttonTop}px`, height: `${vScroller.buttonHeight}px` }" @mousedown="vsbMouseDown">
                    <div v-show="vScroller.runner" class="runner" v-html="vScroller.runner" />
                </div>
            </div>

            <input type="file" ref="importFile" accept=".xlsx, .xls, xlsm, .csv" style="position: absolute; top: 0; left: 0; width: 0; height: 0; opacity: 0; z-index: -1" @keyup="componentTabInto" @change="doImport" />
        </div>
    </div>
</template>
